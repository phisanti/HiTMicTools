...
if __name__ == "__main__":
    ...
    print(f"Range01 test passed: {torch.allclose(test_image, rescaled_range, rtol=1e-2)}")
    ...
    print(f"Z-score test passed: {torch.allclose(test_image, rescaled_zscore, rtol=1e-2)}")
    ...
    print(f"Scaled range for combined method: {scaled_range.min()}, {scaled_range.max()}")
    print(f"Rescaled range for combined method: {rescaled_range.min()}, {rescaled_range.max()}")

    print(f"Combined test passed: {torch.allclose(test_image, rescaled_combined, rtol=1e-2)}")
    ...
    print(f"Fixed range test passed: {torch.allclose(test_image, rescaled_fixed, rtol=1e-4)}")
