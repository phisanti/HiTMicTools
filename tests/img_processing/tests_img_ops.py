...
class TestImgOps(unittest.TestCase):
    def test_detect_and_fix_well(self):
        ...
        result_img_fixed, has_border_fixed = detect_and_fix_well(
            img_with_border.copy(), darkness_threshold_factor=0.5
        )
        ...
        self.assertAlmostEqual(
            result_img_fixed[5, 5], non_border_original_mean, delta=1.0
        )
    ...
    def test_clear_background(self):
        ...
        result_physical_unit = clear_background(
            img.copy(), sigma_r=5, unit="um", pixel_size=0.5, method="subtract"
        )
    ...
    def test_convert_to_uint8(self):
        img_float = (
            np.array([[0.0, 0.5], [0.75, 1.0]], dtype=np.float32) * 200
        )  # Range 0 to 200
        ...
        np.testing.assert_array_equal(
            result, np.array([[0, 127], [191, 255]], dtype=np.uint8)
        )

        img_zero_variance = np.ones((10, 10)) * 50
        with self.assertRaisesRegex(
            ValueError, "Image has zero variance; cannot normalize."
        ):
    ...
    def test_norm_eq_hist(self):
        ...
        img = (
            img / img.max() * 150 + 50
        )  # Ensure values are spread but not full 0-255 initially
    ...
    def test_measure_background_intensity(self):
        ...
        df_bck = measure_background_intensity(
            img_stack.copy(), mask_stack.copy(), target_channel=0, quantile=0.5
        )  # Median

        self.assertIsInstance(df_bck, pd.DataFrame)
        self.assertEqual(len(df_bck), 2)
        self.assertAlmostEqual(
            df_bck.loc[df_bck["frame"] == 0, "background"].iloc[0], 50.0
        )
        self.assertAlmostEqual(
            df_bck.loc[df_bck["frame"] == 1, "background"].iloc[0], 100.0
        )
    ...
    def test_dynamic_resize_roi(self):
        ...
        self.assertTrue(
            torch.allclose(
                resized_small[pad_h_top : pad_h_top + 32, pad_w_left : pad_w_left + 48],
                small_img_torch,
            )
        )
        ...
        first_slice_padded_manually = dynamic_resize_roi(
            img_3d_torch[0].clone(), min_size
        )
