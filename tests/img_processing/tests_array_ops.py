...
class TestArrayOps(unittest.TestCase):
    ...
    def test_stack_indexer(self):
        ...
        np.testing.assert_array_equal(
            stack_indexer([0, 1], 1, range(2)), expected_lists
        )

        # Error handling: negative integers
        with self.assertRaisesRegex(
            ValueError, "Dimensions must be positive integers or lists."
        ):
            stack_indexer(-1, 0, 0)
        with self.assertRaisesRegex(
            ValueError, "All elements in the list dimensions must be positive integers."
        ):
            stack_indexer([-1], 0, 0)
    ...
    def test_convert_image(self):
        ...
        img_float = np.array(
            [[[0.0, 0.5], [0.25, 0.75]], [[0.5, 1.0], [0.75, 0.25]]], dtype=np.float32
        )  # (2,2,2)
        ...
        converted_u8_global = convert_image(
            img_float.copy(), np.uint8, scale_mode="global"
        )
        ...
        np.testing.assert_array_almost_equal(
            converted_u8_global, (img_float * 255).astype(np.uint8)
        )
        ...
        converted_u16_channel = convert_image(
            img_float.copy(), np.uint16, scale_mode="channel"
        )
        ...
        for c in range(img_float.shape[2]):
            ...
            if (
                channel_data_original.min() < channel_data_original.max()
            ):  # Avoid division by zero if flat
                self.assertAlmostEqual(
                    channel_data_converted.min(), 0, delta=1
                )  # allow for rounding
        ...
        img_uint8_input = np.array(
            [[[0, 128], [64, 192]], [[128, 255], [192, 64]]], dtype=np.uint8
        )
        converted_f32_global = convert_image(
            img_uint8_input.copy(), np.float32, scale_mode="global"
        )
        ...
        with self.assertRaisesRegex(
            ValueError, "Unsupported data type: <class 'numpy.int64'>"
        ):
