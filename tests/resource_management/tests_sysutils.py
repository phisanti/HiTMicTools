...
class TestSysUtils(unittest.TestCase):
    ...
    @patch(f"{sysutils_module.__name__}.torch.mps.empty_cache", create=True) # create=True allows mocking even if attr doesn't exist
    @patch(f"{sysutils_module.__name__}.torch.cuda.empty_cache")
    def test_empty_gpu_cache(self, mock_cuda_empty_cache, mock_mps_empty_cache, mock_get_device_for_empty):
        ...
        if hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
            ...
            if hasattr(sysutils_module.torch.mps, "empty_cache"): # Check if the real method exists
    ...
    def test_get_memory_usage(self):
        ...
        cpu_device = torch.device("cpu")
        mem_cpu_used = get_memory_usage(device=cpu_device, free=False, unit="MB", as_string=False)
        self.assertIsInstance(mem_cpu_used, float)
        self.assertGreaterEqual(mem_cpu_used, 0)
        mem_cpu_free_str = get_memory_usage(device=cpu_device, free=True, unit="GB", as_string=True)
        self.assertIsInstance(mem_cpu_free_str, str)
        self.assertTrue(mem_cpu_free_str.endswith(" GB"))

        if torch.cuda.is_available():
            cuda_device = torch.device("cuda")
            mem_cuda_used = get_memory_usage(device=cuda_device, free=False, unit="MB", as_string=False)
            self.assertIsInstance(mem_cuda_used, float)
            # Cannot guarantee >=0 if no tensor is allocated, but it should be a float.
            mem_cuda_free_str = get_memory_usage(device=cuda_device, free=True, unit="GB", as_string=True)
            self.assertIsInstance(mem_cuda_free_str, str)
            self.assertTrue(mem_cuda_free_str.endswith(" GB"))

        if hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
            mps_device = torch.device("mps")
            mem_mps_used = get_memory_usage(device=mps_device, free=False, unit="MB", as_string=False)
            self.assertIsInstance(mem_mps_used, float) # MPS uses system memory reporting like CPU
        ...
        with patch(f"{sysutils_module.__name__}.psutil.Process") as mock_ps_process:
            ...
            mem_other_val = get_memory_usage(device=mock_other_device, unit="MB", as_string=False)
            self.assertAlmostEqual(mem_other_val, 2.0)
            mock_ps_process.assert_called_once()
            
            mem_other_str_gb = get_memory_usage(device=mock_other_device, unit="GB", as_string=True)
            self.assertAlmostEqual(float(mem_other_str_gb.split(' ')[0]), 2.0/1024, places=2)
    ...
    def test_wait_for_memory(self):
        ...
        self.assertTrue(wait_for_memory(required_gb=0.0001, device=test_device, check_interval=0, max_wait=1, logger=mock_logger))
        mock_logger.info.assert_any_call(unittest.mock.ANY) # "Sufficient memory available..."
        mock_logger.reset_mock()

        # 3. Requested memory exceeds total
        if test_device.type == "cuda":
            total_gb = torch.cuda.get_device_properties(test_device).total_memory / (1024 ** 3)
        else: # cpu or mps
            total_gb = psutil.virtual_memory().total / (1024 ** 3)
        
        exceeds_total_gb = total_gb + 1.0

        self.assertFalse(wait_for_memory(required_gb=exceeds_total_gb, device=test_device, logger=mock_logger, raise_on_timeout=False))
        ...
        with self.assertRaisesRegex(MemoryError, expected_msg_exceeds.replace("(", r"\(").replace(")",r"\)")):
            wait_for_memory(required_gb=exceeds_total_gb, device=test_device, logger=mock_logger, raise_on_timeout=True)
        mock_logger.error.assert_called_once_with(expected_msg_exceeds)
        mock_logger.reset_mock()
        ...
        realistic_required_gb = min(total_gb * 0.8, 1.0) # Request 80% of total or 1GB, whichever is smaller
        mock_free_mem_val = realistic_required_gb / 2 

        with patch(f"{sysutils_module.__name__}.get_memory_usage", return_value=mock_free_mem_val) as mock_get_mem:
            self.assertFalse(wait_for_memory(required_gb=realistic_required_gb, device=test_device, 
                                             check_interval=0, max_wait=1, logger=mock_logger, raise_on_timeout=False))
            ...
            timeout_error_call = [c for c in mock_logger.error.call_args_list if "Memory wait timeout" in c[0][0]]
            self.assertEqual(len(timeout_error_call), 1)
            self.assertIn(f"free: {mock_free_mem_val:.2f} GB", timeout_error_call[0][0][0])
            self.assertIn(f"required: {realistic_required_gb} GB", timeout_error_call[0][0][0])
            mock_logger.reset_mock()

            with self.assertRaisesRegex(MemoryError, "Memory wait timeout"):
                wait_for_memory(required_gb=realistic_required_gb, device=test_device, 
                                check_interval=0, max_wait=1, logger=mock_logger, raise_on_timeout=True)
            self.assertTrue(mock_logger.warning.called)
            timeout_error_call_raise = [c for c in mock_logger.error.call_args_list if "Memory wait timeout" in c[0][0]]
        ...
        with patch(f"{sysutils_module.__name__}.get_memory_usage") as mock_get_mem_prog:
            ...
            mock_get_mem_prog.side_effect = [mock_free_mem_val, mock_free_mem_val, realistic_required_gb + 0.1]
            self.assertTrue(wait_for_memory(required_gb=realistic_required_gb, device=test_device, 
                                            check_interval=0, max_wait=5, logger=mock_logger, raise_on_timeout=False))
            self.assertEqual(mock_logger.warning.call_count, 2)
            
            success_info_call = [c for c in mock_logger.info.call_args_list if "Sufficient memory available" in c[0][0]]
            self.assertEqual(len(success_info_call), 1)
            self.assertIn(f"{realistic_required_gb + 0.1:.2f} GB", success_info_call[0][0][0])
            mock_logger.reset_mock()

        # 6. Test with no logger (should run without error)
        with patch(f"{sysutils_module.__name__}.get_memory_usage", return_value=0.0002): # Ensure it passes
             self.assertTrue(wait_for_memory(required_gb=0.0001, device=test_device, check_interval=0, max_wait=1, logger=None))
