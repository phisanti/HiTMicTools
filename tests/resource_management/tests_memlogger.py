...
class TestMemoryLoggerConcise(unittest.TestCase):
    ...
    def test_log_vram_gpu_available(self, mock_dev, mock_get_mem):  # VRAM, GPU on
        ...
        self.assertIn(
            "M | VRAM: 2.50 GB used / 5.50 GB free", self.log_stream.getvalue()
        )
        mock_get_mem.assert_has_calls(
            [
                call(device=mock_dev(), unit="GB", as_string=False),
    ...
    def test_log_ram_and_vram(self, mock_dev, mock_get_mem):  # RAM and VRAM
        ...
        self.assertIn(
            "M | RAM: 1.50 GB used | VRAM: 2.50 GB used / 5.50 GB free",
